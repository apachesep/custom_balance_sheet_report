...
class common_report_header(object):
    ...
    def _get_fiscalyear(self, data):
        if data.get('form', False) and data['form'].get('fiscalyear_id', False) and 'without_con' in data['form']['used_context']:
            return filter(str.isdigit, str(self.pool.get('account.fiscalyear').browse(self.cr, self.uid, data['form']['fiscalyear_id'][0]).name))
        elif data.get('form', False) and data['form'].get('fiscalyear_id', False):
